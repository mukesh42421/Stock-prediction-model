...
@app.route("/predict_high",methods=["POST"])
def predict_high():
    
    int_features = [int(i) for i in request.form.values()]
    final_feature = [np.array(int_features)]
    prediction = model_high.predict(final_feature)
    
    output = round(prediction[0],2)
    
    return render_template("index.html",prediction_text = f"Predictive high will be {output}")


@app.route("/predict_low",methods=["POST"])
def predict_low():
    
    int_features = [int(i) for i in request.form.values()]
    final_feature = [np.array(int_features)]
    prediction = model_low.predict(final_feature)
